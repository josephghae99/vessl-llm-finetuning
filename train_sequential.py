...
for i, dataset_path in enumerate(datasets):
    ...
    if i == 0:
        ...
        model = AutoModelForCausalLM.from_pretrained(f"/artifacts/checkpoint-{i}", use_cache=False, device_map="auto",
                                                     load_in_4bit=True)

    training_args = TrainingArguments(
        output_dir=f"/artifacts/checkpoint-{i + 1}",
        per_device_train_batch_size=4,
        gradient_accumulation_steps=2,  # Modifié : batch size effective de 8
        num_train_epochs=3,  # Modifié : 3 epochs par dataset
        warmup_steps=100,
        logging_steps=10,
        save_strategy="epoch",
        learning_rate=2e-5,
